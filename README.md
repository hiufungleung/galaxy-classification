# Galaxy Morphology Classification — DATA7901/7903 (UQ)
...
│   │   ├── dr19.csv           # 507k rows, 723 columns (main table)
...
> **GPU note:** On Windows/Linux, `torch` and `torchvision` are installed from the PyTorch CUDA 12.8 index. On macOS (Apple Silicon), PyPI torch with MPS support is used automatically. If you need a different CUDA version, edit the `pytorch-cu128` index URL in `pyproject.toml` and re-run `uv lock`.

> **macOS note:** XGBoost requires the OpenMP runtime, which is not included with macOS. Install it with Homebrew before running the pipeline: `brew install libomp`
...
```bash
{ head -1 MERGED1.csv; tail -n +2 MERGED1.csv; tail -n +2 MERGED2.csv; tail -n +2 MERGED3.csv; } > dr19.csv
```

### Step 2 — Download galaxy images and spectra

Image and spectra download scripts were used during data acquisition and are no longer included in the repository. The downloaded data resides in `input/images/` (JPEG cutouts) and `input/spectra/` (FITS files), both gitignored.

### Cloning to a new machine

Data files are gitignored (too large for git). To run the pipeline on a new machine, copy the `input/` directory from the original machine:

```bash
# From the original machine (e.g., via rsync, scp, or external drive):
rsync -avP input/ <new-machine>:galaxy-classification/input/

# Required contents:
#   input/tables/dr19.csv   (~250 MB)
#   input/images/*.jpeg                      (~4 GB, 214k files)
#   input/spectra/*.fits                     (~2 GB, 142k files)
```

Stage 0 of the pipeline (`src.data.prepare`) generates all derived CSVs (`labeled_index.csv`, `working_set.csv`, `split_*.csv`) from the raw merged CSV and the downloaded images/spectra.
...
| --- | --- | --- | --- |
| 0. Prepare data | `src.data.prepare` | `labeled_index.csv`, `working_set.csv`, `split_*.csv` | CPU, ~2 min |
...
| 8. Evaluation summary | `src.evaluate` | `checkpoints/results_summary.csv` | CPU, <1 min |
...
| Image (ResNet-18) | 0.807 | 95.5% | 0.963 | 0.958 | 0.501 |
| Spectral (1D-CNN) | 0.563 | 77.7% | 0.770 | 0.800 | 0.118 |
| **Fusion** | **0.873** | **96.7%** | **0.972** | **0.968** | **0.679** |
...
47 tabular features from `PhotoObj` + `SpecObj`. Key groups:
...
Tabular (47 features) → MLP encoder       → 64-d
...
**`input/tables/dr19.csv` not found**
...
**`ModuleNotFoundError: No module named '_lzma'` (macOS / pyenv)**
Python was built without `lzma` support. Install `xz` and rebuild Python:

```bash
brew install xz
pyenv install 3.12.12   # or your Python version
```

Then re-run `uv sync` in the project directory.
