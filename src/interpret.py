...
def main():
    ...
    parser.add_argument("--only", choices=["feature-importance", "gradcam"],
                        help="Run only this analysis (default: both)")
    args = parser.parse_args()

    if args.only != "gradcam":
        xgb_feature_importance()
    if args.only != "feature-importance":
        try:
            gradcam_grid()
        except ImportError:
