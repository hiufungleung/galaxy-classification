...
def main():
    ...
    # num_workers=0 for test: avoids Windows pagefile exhaustion after long training
    ...
    suffix = f"_config{args.config}" if args.config else "_full"
    ...
    # ── Test evaluation ───────────────────────────────────────────────────
    print(f"Loading best checkpoint: {ckpt_path}", flush=True)
