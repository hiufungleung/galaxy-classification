...
def run_epoch(
    model, loader, criterion, optimizer, device, train: bool, ablate: str = ""
):
...
def extract_encoder_features(model, loader, device):
    ...
    with torch.no_grad():
        for img, spec, tab, has_spec, labels in tqdm(
...
def make_cached_loader(
    img_feats, spec_feats, tabs, has_specs, labels, batch_size, shuffle
):
...
def main():
    ...
    parser.add_argument("--data", default=str(TABLES / "labeled_index.csv"))
    parser.add_argument("--batch-size", type=int, default=BATCH_SIZE)
    parser.add_argument(
        "--img-ckpt",
        default=str(IMG_CKPT),
        help="Path to pretrained image encoder checkpoint",
    )
    parser.add_argument(
        "--spec-ckpt",
        default=str(SPEC_CKPT),
    ...
    parser.add_argument(
        "--ablate",
        default="",
        choices=["", "image", "spectral", "tabular"],
    ...
    # ── Build feature table ───────────────────────────────────────────────
    # Default: labeled_index.csv (already renamed + labeled by prepare.py)
    ...
    def load_split(path):
        ...
        merged["spec_path"] = merged.apply(
            lambda r: str(
                SPEC_DIR
                / f"spec-{int(r.plate)}-{int(r.spec_mjd)}-{int(r.fiberid):04d}.fits"
            ),
            axis=1,
        )
        return merged
    ...
    print(
        f"Fusion rows — train: {len(train_df):,}  val: {len(val_df):,}  test: {len(test_df):,}"
    )
    ...
    # Release persistent workers from train/val loaders to free memory before extraction
    del train_loader, val_loader

    extract_train = DataLoader(
        train_ds,
        batch_size=args.batch_size,
        shuffle=False,
        num_workers=NUM_WORKERS,
    ...
    print("  Train set:")
    tr_img, tr_spec, tr_tab, tr_hs, tr_lbl = extract_encoder_features(
        model, extract_train, device
    )
    print("  Val set:")
    va_img, va_spec, va_tab, va_hs, va_lbl = extract_encoder_features(
        model, extract_val, device
    )
    ...
    cached_train_loader = make_cached_loader(
        tr_img, tr_spec, tr_tab, tr_hs, tr_lbl, BATCH_SIZE_P1, shuffle=True
    )
    cached_val_loader = make_cached_loader(
        va_img, va_spec, va_tab, va_hs, va_lbl, BATCH_SIZE_P1, shuffle=False
    )
    ...
    for epoch in range(1, PHASE1_EPOCHS + 1):
        train_loss, _ = run_epoch_cached(
            model, cached_train_loader, criterion, optimizer, device, train=True
        )
        val_loss, val_res = run_epoch_cached(
            model, cached_val_loader, criterion, optimizer, device, train=False
        )
        print(
            f"  Phase1 epoch {epoch:2d}  train_loss={train_loss:.4f}  val_f1={val_res['macro_f1']:.4f}"
        )
    ...
    del cached_train_loader, cached_val_loader, extract_train, extract_val

    # ── Phase 2: unfreeze all, joint fine-tuning ──────────────────────────
    print("\n── Phase 2: all params unfrozen, joint fine-tuning ──")

    # Recreate train/val loaders with persistent workers for Phase 2
    train_loader = DataLoader(
        train_ds,
        batch_size=args.batch_size,
        shuffle=True,
    ...
    for epoch in range(1, MAX_EPOCHS + 1):
        ...
        print(
            f"  Epoch {epoch:3d}  train_loss={train_loss:.4f}  val_loss={val_loss:.4f}  val_f1={val_f1:.4f}"
        )
