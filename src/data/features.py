...
def engineer_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    Note: lnLDeV/lnLExp dropped — 75-84% sentinel in the data; deV_exp_ratio removed.
    """
    df = df.copy()
...
def handle_missing(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # Coerce to numeric (some columns load as object due to mixed types) then replace sentinels
